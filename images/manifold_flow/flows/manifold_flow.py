...
class ManifoldFlow(BaseFlow):
    ...
    def forward(self, x, mode="mf", context=None, return_hidden=False):
        ...
        assert mode in ["mf", "dnf", "pie", "slice", "projection", "pie-inv", "mf-fixed-manifold"]
        ...
        # Decode
        if mode == "dnf":
            x_reco, inv_log_det_inner, inv_log_det_outer, inv_jacobian_outer, h_manifold_reco = self._decode(h_manifold, mode=mode, context=context)
    ...
    def normal_sampling(self,x,context=None):
        """ Sampling from normal space by setting v ~ N(0,sig^2) """
        
        u, h_manifold, _, _, _ = self._encode(x, context)
        h_orthogonal = self.orthogonal_latent_distribution.sample(x.shape[0], context=None)
        h = self.projection.inverse(h_manifold, orthogonal_inputs=h_orthogonal)
        x_normal, _ = self.outer_transform.inverse(h, full_jacobian=False, context=context if self.apply_context_to_outer else None)
	
	#x_normal = self.decode(h_manifold, u_orthogonal=h_orthogonal, context=None)
        return x_normal
    ...
    def sample_img_path(self, x, n=1, context=None):
        u, _, _, _, _ = self._encode(x, context)
        dt = torch.linspace(0,1,n) 
        u_t = torch.zeros([n,self.latent_dim])
        for k in range(n):
            u_t[k] = u[0,:]*dt[k] + (1-dt[k])*u[1,:]
        x, _, _, _, _ = self._decode(u_t, mode="projection", u_orthogonal=None, context=context)   
        return x
    ...
    def _decode(self, u, mode, u_orthogonal=None, context=None):
        ...
        if mode in ["pie", "slice", "projection", "mf-fixed-manifold", "dnf"]:
    ...
    def _log_prob(self, mode, u, h_orthogonal, log_det_inner, log_det_outer, inv_log_det_inner, inv_log_det_outer, inv_jacobian_outer):
        if mode in  ["pie", "dnf"]:
            ...
            #print('log_det_outer',log_det_outer)
            #print('log_det_inner',log_det_inner)
            log_prob = log_prob + log_det_outer + log_det_inner
            #criterion = torch.sum(h_orthogonal**2,dim=1)> 0.01/500
