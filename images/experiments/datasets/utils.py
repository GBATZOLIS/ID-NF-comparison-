...
class Preprocess:
    def __init__(self, num_bits, noise_type, sig2, scale_factor):
    ...
    def __call__(self, img):
        ...
        if self.noise_type == 'uniform':
            img = img + np.sqrt(self.sig2) * torch.rand_like(img)
        elif self.noise_type == 'gaussian':
            img = img + np.sqrt(self.sig2) * torch.randn_like(img)
      
        # shape dim of tensor
        if self.scale_factor != 1:
            img = img[None,:]
            img = torch.nn.functional.interpolate(img, scale_factor=self.scale_factor )  
            img = img.squeeze()
        return img
