...
def get_singular_values(filename):
    """ Saves model checkpoints. """
    ...
    def callback(i_epoch, model, loss_train, loss_val, subset=None, trainer=None, last_batch=None):
        ...
        if (i_epoch + 1) % 50 == 0: #50
            x = last_batch["x"]
            x = x[0:1].float() 
            x_ = torch.autograd.Variable(x)
            jac_ = torch.autograd.functional.jacobian(model.encode,x_)
            jac_mat = jac_.reshape([3072,3072])
            U,S_x,V = torch.svd(jac_mat)
            S_np = S_x.detach().cpu().numpy()
            np.save(filename.format(i_epoch + 1), S_np)
            print("Singular values * 256 < 1 = ", np.sum(S_np * 256 < 1))
            # torch.save(model.state_dict(), filename.format(i_epoch + 1))
